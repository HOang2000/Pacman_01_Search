        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if nextState in self.corners and nextState in unvisitedCorners:
                    ...
                    unvisitedCorners = tuple(remaining)
                cost = 1
...
def cornersHeuristic(state, problem):
    ...
    while len(unvisitedCorners) > 0:
        distanceToCorner = 999999
        for i in range (0, len(unvisitedCorners)):
            if util.manhattanDistance(position, unvisitedCorners[i]) < distanceToCorner:
                closestCorner = unvisitedCorners[i]
                distanceToCorner = util.manhattanDistance(position, unvisitedCorners[i])
        position = closestCorner
        heuristicValue += distanceToCorner
